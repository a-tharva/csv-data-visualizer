...
def get_column_name_from_unique_value(unique_value: str, unique_values_dict: dict[str, list[str]]) -> str:
    ...
    if label == 'Filter':
        for column_name, radio_button in radio_buttons.items():
            if isinstance(radio_button, RadioButtons) and radio_button.value_selected != 'None':
                column_name = get_column_name_from_unique_value(radio_button.value_selected, unique_values_dict)
